...
describe("App integration and regression flows", () => {
  ...
  it("logs in and renders the administrative layout with role-based navigation", async () => {
    ...
    expect(screen.getByRole("button", { name: "Países" })).toBeInTheDocument();
    ...
    expect(fetchMock).toHaveBeenCalledWith("http://localhost:5080/api/Auth/login", expect.any(Object));
    expect(fetchMock).toHaveBeenCalledWith("http://localhost:5080/api/admin/users/me", expect.any(Object));
  });
  ...
  it("creates, updates, rejects duplicate and deletes countries", async () => {
    ...
    renderApp();
    await login();
    await userEvent.click(screen.getByRole("button", { name: "Países" }));
  ...
  it("creates, filters, updates, rejects invalid country and deletes departments", async () => {
    ...
    renderApp();
    await login();
    await userEvent.click(screen.getByRole("button", { name: "Departamentos" }));
  ...
  it("creates users, changes roles and deactivates users", async () => {
    ...
    await userEvent.click(screen.getAllByRole("checkbox", { name: "Países ID 4" })[0]);
    ...
    await userEvent.click(screen.getAllByRole("checkbox", { name: "Usuarios ID 6" })[1]);
